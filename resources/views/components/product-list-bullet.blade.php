...
<article class="mx-8 flex flex-col gap-10">
    @foreach ($categories as $category)
        {{--カテゴリーのroleが1でない、またはそのカテゴリーに属する商品がない場合はスキップ--}}
        @if($category->items->isEmpty() || $category["role"] !== 1)
            @continue
        @endif
                ...
                @foreach($category->items as $item)
                    ...
                    <div class="w-full md:w-[45%] lg:w-[20%] flex flex-col p-4 border rounded">
                        <aside class="w-full flex flex-col items-start">
                            ...
                            <button
                                onclick="OpenBulletOtherModal(event)"
                                data-item="{{json_encode($item)}}"
                                data-category="{{json_encode($item->category)}}"
                                class="border border-solid border-gray-800 px-4 py-1 rounded-sm mt-1"
                            >
                                詳細
                            </button>
                        </aside>
