<x-app-layout>
        ...
        <div class="relative p-4 w-[80%] h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700 w-full">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
                    {{--品名--}}
                    <h3 id="other_modal_title" class="text-xl font-semibold text-gray-900 dark:text-white"></h3>
                    {{--閉じるボタン--}}
                    <button type="button" id="other_edit_close_btn" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="static-modal">
                ...
                <div class="p-4 md:p-5 space-y-4 w-full">
                    <form enctype="multipart/form-data" method="post" action="{{route("admin.other.update")}}"  class="w-[90%] flex flex-col gap-2">
                        ...
                        <x-dash-form-component :flag="true" title="品名">
                            <input type="text" id="other_modal_item_name" required placeholder="例）ジャケット" name="name" class="rounded-lg">
                        ...
                        <x-dash-form-component :flag="false" title="備考欄">
                            <input type="text" id="other_modal_note" placeholder="例）サイズS,M,Lあり" name="note" class="rounded-lg">
                        </x-dash-form-component>
                        {{--ID--}}
                        <input type="hidden" name="id" id="other_modal_id">
                        {{--登録ボタン--}}
    ...
    <div id="other_modal_backdrop" class="hidden bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-40"></div>

    <script>
        const otherModal = document.getElementById('other_edit_modal');//モダル要素
        const closeOtherModalButton = document.getElementById('other_edit_close_btn');//モーダルクローズボタン
        const otherModalBackdrop = document.getElementById('other_modal_backdrop');//モダルオープン時のグレー背景

        // モーダルを閉じる
        closeOtherModalButton.addEventListener('click', () => {
            otherModal.classList.add('hidden');  // モーダルを非表示
            otherModalBackdrop.classList.add('hidden');  // バックドロップを非表示
        });

        function OpenOtherEditModal(e){
            // モダルを開く
            otherModal.classList.remove('hidden');  // モーダルを表示
            otherModal.setAttribute('aria-hidden', 'false');
            otherModalBackdrop.classList.remove('hidden');  // バックドロップを表示
            ...
            // 品名TOP
            const modalTitle = document.getElementById('other_modal_title');
            modalTitle.textContent = `品名：${data.name} `
            // 品名input
            const modalItemName = document.getElementById('other_modal_item_name');
            ...
            // ID
            const modalId = document.getElementById('other_modal_id');
